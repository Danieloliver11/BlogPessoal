...
public class UsuarioService {
	...
	public Optional<Usuario> CadastrarUsuario(Usuario usuario) {
		...
		if(repository.findByUsuario(usuario.getUsuario()).isPresent() && usuario.getId() == 0) //verifica se esse usuario já existe
			//&& usuario.getId() == 0 atualizado 22
			return null;
	...
	public Optional<UserLogin> Logar(Optional<UserLogin> user){
		...
		if(usuario.isPresent()) {
			...
			if(encoder.matches(user.get().getSenha(), usuario.get().getSenha())) {
				...
				// AQUI PODEMOS VER QUE estamos pegando os dados do Usuario model e passando para o UserLogin. 
				user.get().setNome(usuario.get().getNome());
				user.get().setSenha(usuario.get().getSenha());
				
				//add na aula front-end (para poder aparecer para nosso usuario no front atraves do UsuarioLogin)
